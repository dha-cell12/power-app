...
const getDriverPath = (windowData?: DB.Window) => {
  ...
  // If local Chrome path is specified for the window, use it automatically
  if (windowData?.localChromePath) {
    return windowData.localChromePath;
  }

  // If Chromium path is specified for the window
  ...
  try {
    ...
    if (windowData.status === 2 && windowData.port) {
      ...
      try {
        ...
        // If browser information is successfully retrieved, the window is still available
        if (data) {
          logger.info(`Window ${id} is already running on port ${windowData.port}`);
        ...
        // If retrieval fails, the window is actually closed despite being marked as open
    ...
    if (driverPath) {
      ...
      if (headless) {
        ...
        if (!isMac) {
          launchParamter.push('--disable-gpu'); // This parameter is not needed on Mac
        }
      } else {
        launchParamter.push('--new-window');
      }

      // Add user-defined launch parameters
      ...
      // Add debug parameters (if needed)
      ...
      // Need to listen to stderr here, otherwise it might hang on some websites
        ...
        if (chromeInstance.pid) {
          try {
            if (process.platform === 'win32') {
              try {
                // Use chcp 65001 to set console code page to UTF-8
              ...
              // Unix system handling remains unchanged
              try {
...
/**
 * Focus the specified window and bring it to the front
 */
  ...
  try {
    ...
    if (data) {
      ...
      if (pages.length > 0) {
        ...
        // First use bringToFront for basic topping
