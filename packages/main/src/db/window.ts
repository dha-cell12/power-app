...
const all = async () => {
  return await db('window')
    .select(
      ...
      'window.ua',
      'window.status',
      'window.useLocalChrome',
      'window.localChromePath',
      'window.chromiumBinPath',
...
const getById = async (id: number) => {
  // Get window record and its associated data
  ...
  if (windowData.tags) {
    ...
    // Add tag names to the return result
...
const getByPid = async (pid: number) => {
  // Get window record and its associated data by PID
  const windowData = await db('window')
    .select(
      'window.*',
      'group.name as group_name',
...
const create = async (windowData: DB.Window, fingerprint?: SafeAny) => {
  ...
  // If local Chrome is not selected, set chromiumBinPath to null and use global settings
  if (!windowData.useLocalChrome) {
    windowData.chromiumBinPath = null;
  }

  // Use UTC ISO string to store created_at, ensuring timezone consistency
  const insertData = {
    ...windowData,
    created_at: new Date().toISOString(),
  };
  const [id] = await db('window').insert(insertData);
  return {
    success: true,
