...
const all = async () => {
  return await db('proxy')
    .leftJoin('window', function () {
      this.on('window.proxy_id', '=', 'proxy.id').andOn('window.status', '>', 0 as SafeAny); // Added filtering conditions
    })
    .select('proxy.*')
...
const batchDelete = async (ids: number[]) => {
  ...
  // If there are referenced IDs, you can choose to throw an error or return relevant information
